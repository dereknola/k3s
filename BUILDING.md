...
To build the full release binary for your local platform, you may now run `make local-binary`, which will create `./dist/artifacts/k3s`.

To build the full release platform set with Docker Buildx, run:

```bash
make local-multiarch-binary
```
