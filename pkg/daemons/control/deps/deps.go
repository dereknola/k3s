...
func genEncryptionConfigAndState(controlConfig *config.Control) error {
	...
	var keyName string
	switch controlConfig.EncryptKeyType {
	case secretsencrypt.AESCBCKeyType:
		keyName = "aescbckey"
	case secretsencrypt.SecretBoxKeyType:
		keyName = "secretboxkey"
	default:
		return fmt.Errorf("unsupported secrets-encryption-key-type %s", controlConfig.EncryptKeyType)
	}
	...
	if controlConfig.EncryptKeyType == secretsencrypt.AESCBCKeyType {
	...
	} else if controlConfig.EncryptKeyType == secretsencrypt.SecretBoxKeyType {
		provider = []apiserverconfigv1.ProviderConfiguration{
			{
